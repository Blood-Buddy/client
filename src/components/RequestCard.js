        ...
        <View>
            {data?.map((item) => (
                item.totalRequest > item.totalCollected && (
                    <View key={item?._id} className="mt-4 bg-white rounded-lg shadow-md">
                        ...
                        <Text className='mt-2 mx-6 '>
                            Blood type B needed: {item?.bloodType?.B?.request},
                            collected: {item?.bloodType?.B?.collected}
                        </Text>
                        <Text className='mt-2 mx-6 '>
                            Blood type AB needed: {item?.bloodType?.AB?.request},
                            collected: {item?.bloodType?.AB?.collected}
                        </Text>
                        <Text className='mt-2 mx-6 '>
                            Blood type O needed: {item?.bloodType?.O?.request},
                            collected: {item?.bloodType?.O?.collected}
                        </Text>
                        ...
                        <View className="flex-row-reverse">
                            ...
                            >
                                <Text className="text-[#f2f2f2] font-bold">
                                    Book Appointment
                                </Text>
                            </TouchableOpacity>
                        </View>
            ...
            >
                <SafeAreaView>
                    ...
                    {selectedItem && (
                        <View className='mx-2'>
                            <Text className='mt-3 mb-3 text-black'>
                                Before making an appointment, make sure you meet the following requirements:
                            </Text>
                            ...
                            <TouchableOpacity
                                ...
                                onPress={() => {
                                    handleCloseModal();
                                    navigation.navigate("Book Appointment", { requestData: selectedItem });
                                }}
